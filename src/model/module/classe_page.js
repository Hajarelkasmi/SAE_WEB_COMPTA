...
module.exports = (app) => {
    app.get('/api/classe_pages', async (req, res) => {
        try {
            ...
            if (page_id) {
                const classe_pages = await Classe_Page.findAll({ where: { page_id } });
    ...
    app.delete('/api/classe_pages/:id_classe/:id_page', verifyToken, verifyAdmin, async (req, res) => {
        try {
            const classe_page = await Classe_Page.findOne({
                where: {
                    classe_id: req.params.id_classe,
                    page_id: req.params.id_page
                }
            });
            if (classe_page) {
